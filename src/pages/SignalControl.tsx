...
export default function SignalControl() {
  ...
  const applyOptimization = async (id: string) => {
    try {
      await fetch(`http://localhost:8000/optimize-signal?intersection_id=${id}`, { method: 'POST' });
    } catch (e) {
      console.error(e);
    }
  };

  const selectedIntersection = data.find(i => i.id === selectedId) ?? data[0] ?? initialIntersections[0];
